...
const Sidebar = ({ mobile = false, onNavigate }) => {
  ...
  return (
    <AnimatePresence>
      <motion.aside
        animate={{ width: mobile ? '100%' : sidebarCollapsed ? 72 : 260 }}
      ...
      >
        ...
        <div className="px-3 py-3">
          <motion.button
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl bg-gradient-to-r from-primary-600 to-purple-600 text-white font-medium transition-all hover:shadow-neon ${!mobile && sidebarCollapsed ? 'justify-center' : ''}`}
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
          >
            <HiUpload className="text-lg flex-shrink-0" />
            {(mobile || !sidebarCollapsed) && <span>Upload Files</span>}
          </motion.button>
        </div>
          ...
          {navItems.map((item, idx) => {
            ...
            return (
              <NavLink
                ...
                onClick={onNavigate}
                className={({ isActive }) =>
                  `sidebar-link ${isActive ? 'active' : ''} ${!mobile && sidebarCollapsed ? 'justify-center' : ''}`
                }
              >
          ...
          {user?.role === 'admin' && (
            <>
              ...
              >
                ...
                {(mobile || !sidebarCollapsed) && <span className="text-sm sidebar-label">Admin Panel</span>}
              </NavLink>
            </>
          )}
        </nav>
        ...
        {/* Storage */}
        {(mobile || !sidebarCollapsed) && storageData && (
        ...
        <div className="border-t border-slate-100 dark:border-dark-800 p-3">
          <div className={`flex items-center gap-3 ${!mobile && sidebarCollapsed ? 'justify-center' : ''}`}>
            <img
